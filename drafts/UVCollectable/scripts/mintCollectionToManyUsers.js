...
const mintCollectionToManyUsers = () => {
  const to = [
  ...
  nft
    .mintCollectionToManyUsers(eventId, to, false, 0)
    .then((tx) => tx.wait(1))
    .then((receipt) => console.log(`Your transaction is confirmed, its receipt is: ${receipt.transactionHash}`))
    .catch((e) => console.log("something went wrong", e));
};

mintCollectionToManyUsers();

// npx hardhat run scripts/mintCollectionToManyUsers.js --network mumbai
